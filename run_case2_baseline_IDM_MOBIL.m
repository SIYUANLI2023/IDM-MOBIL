...
cfg.sv1_follow.enter_margin = -8.0; % hysteresis enter threshold
...
for n = 1:N
    ...
    % —— Trigger detection: if EV (or SV2) just started a lane change, let SV1 accelerate 
    ...
    if (~veh(2).accel_override.active) && ...
       ( (cfg.sv1_trigger_on_EV  && ev_started) || ...
         (cfg.sv1_trigger_on_SV2 && sv2_started) )
        veh(2).accel_override.active = true;
        veh(2).accel_override.t0     = t;
    end
