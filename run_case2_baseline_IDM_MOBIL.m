...
cfg.Tsim    = 45.0;         % total simulation time (s)
...
for i = 1:numel(veh)
    veh(i).log.t(1)    = 0;
    veh(i).log.x(1)    = veh(i).x;
    veh(i).log.y(1)    = veh(i).y;
    veh(i).log.v(1)    = veh(i).v;     % 这里就是 30
    veh(i).log.ax(1)   = 0;
    veh(i).log.lane(1) = veh(i).lane;
    veh(i).log.alat(1) = 0;
end
